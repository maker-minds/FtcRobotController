...
@Autonomous(name = "Autonomous_EAGoose_blue_close", group = "autonomous")
public class Autonomous_EAGoose_blue_close extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        encoderDrive(TURN_SPEED, 3, 3, -3, -3 ,3);
        revolver.togglemode();
        ...
        pusher.setPosition(0.5);
        sleep(1500);
        encoderDrive(TURN_SPEED,8, 8, -8, -8, 5);
    ...
    public void encoderDrive(double speed, double backleftInches, double frontleftinches, double backrightInches, double frontrightinches, double timeout) {
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() && (runtime.seconds() < timeout) &&
                ...
                int diference = Math.abs(DriveTrain.leftback.getTargetPosition() - DriveTrain.leftback.getCurrentPosition());
                if(diference < 50) {
                    DriveTrain.setPowerleft(map(diference, 1, 50, 0.3, speed));
                }
                diference = Math.abs(DriveTrain.rightback.getTargetPosition() - DriveTrain.rightback.getCurrentPosition());
                if(diference < 50) {
                    DriveTrain.setPowerleft(map(diference, 1, 50, 0.3, speed));
                }
            }
